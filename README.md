...
- **Real SQL sources**, compile-time selectable: SQLite (always compiled in), Postgres (on by default), MySQL/MariaDB (opt-in, `--features mysql`), and Microsoft SQL Server (opt-in, `--features mssql`) today, each driver behind its own Cargo feature so a binary only carries the driver(s) it was built with.
...
SQLite is always compiled in — every build of this binary has it, `--no-default-features` included. Postgres is on by default; MySQL/MariaDB and Microsoft SQL Server are additional, opt-in driver features (neither is part of `default`):
...
cargo build --release --all-features               # sqlite + postgres + mysql/mariadb + mssql
```
...
├── sql/          # the SqlDriver trait + postgres/sqlite/mysql/mssql adapters
