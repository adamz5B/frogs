...
- **Real SQL sources**, compile-time selectable: SQLite (always compiled in), Postgres (on by default), MySQL/MariaDB (opt-in, `--features mysql`), Microsoft SQL Server (opt-in, `--features mssql`), and Oracle (opt-in, `--features oracle`) today, each driver behind its own Cargo feature so a binary only carries the driver(s) it was built with.
...
- **Service registry**: an optional `config/services.json` (name → base URL), on only when `features.serviceRegistry` is set, lets an HTTP source's `url`/`body` reference a logical name — `{{services.pricing}}` — instead of a hardcoded base URL, resolved through the same `{{param}}` templating every other value already uses.
...
SQLite is always compiled in — every build of this binary has it, `--no-default-features` included. Postgres is on by default; MySQL/MariaDB, Microsoft SQL Server, and Oracle are additional, opt-in driver features (none is part of `default`):

```sh
cargo build --release                              # default: sqlite + postgres
cargo build --release --no-default-features        # sqlite only, no postgres/TLS at all
cargo build --release --features mysql             # sqlite + postgres + mysql/mariadb
cargo build --release --features mssql             # sqlite + postgres + mssql
cargo build --release --features oracle            # sqlite + postgres + oracle
cargo build --release --all-features               # sqlite + postgres + mysql/mariadb + mssql + oracle
```
