...
impl fmt::Display for SecurityLoadError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SecurityLoadError::MissingVerifierFile { scheme, file } => write!(f, "security scheme '{scheme}' names verifier file {} which doesn't exist", file.display()),
            SecurityLoadError::InvalidValidIf { scheme, file, source } => write!(f, "verifier {} (scheme '{scheme}') has an invalid validIf: {source}", file.display()),
        }
    }
}
...
impl SecurityConfig {
    ...
    pub fn load(security_dir: &Path) -> Result<Self, SecurityLoadError> {
        ...
        let schemes: HashMap<String, SchemeConfig> = match fs::read_to_string(&schemes_path) {
            Ok(contents) => serde_json::from_str(&contents).map_err(|source| SecurityLoadError::Parse {
                file: schemes_path.clone(),
                source,
            })?,
        ...
        for (scheme_name, scheme) in &schemes {
            ...
            let def: VerifierDef = serde_json::from_str(&contents).map_err(|source| SecurityLoadError::Parse {
